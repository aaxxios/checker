...
def split_into_chunks(
    text, num_words=12, ignore_chunks_below_num_words=5, filter_poor_quality=False
):
    chunks = []
    ...
    for x in range(0, iterate_times):
        ...
        if (
            filter_poor_quality
            and len(remove_special_characters(chunk)) / len(chunk) < 0.5
        ):
            add_chunk = False
...
def calculate_unique_score_for_chunk(
    chunk, stop_words=stop_words(), stop_phrases=stop_phrases()
):
    ...
    words = chunk.split()
    num_stopword_intersection = len(
        [word for word in words if word.lower() in stop_words]
    )
    num_words = len(words)
...
def html_to_basic_text(html:str):
    try:
        soup = BeautifulSoup(html, choose_bs_parser())
...
def amend_filepath_slashes(path: str):
